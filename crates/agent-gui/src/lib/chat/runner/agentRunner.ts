  ...
  if (has("Image")) {
    sections.push(
      [
        ...
        "- Image source forms are mutually exclusive. For a local generated output, call Image with exactly one path or paths field; never combine source, path, url, or base64 fields and never invent a placeholder URL.",
  ...
  if (hasAny("ImageManager", "ImageGenerate", "ImageEdit")) {
    sections.push(
      [
        ...
        '- For an explicit request to clear only a custom relay protocol, use ImageManager(action="clear_adapter") and retain the connection. For an explicit request to reset or remove api2img/image-service configuration, use ImageManager(action="clear_configuration"). It clears the API key, adapter, URL, models, mode, and timeout but keeps generated images and installed pets.',
        ...
        "- Native image adapters automatically inspect JSON and SSE responses for image URLs/Base64 images and common async task/status fields. Do not require the user to supply JSON extraction paths. When an image job fails, inspect its returned diagnostic, update the request template or only the needed adapter hint, then retry the original requested operation once.",
